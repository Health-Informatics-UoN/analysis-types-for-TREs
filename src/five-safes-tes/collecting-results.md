...
These instructions assume you have [submitted a task](submitting-to-5s-tes). You will normally collect the results in this way if you have submitted a task using the wizards, but can do it at any time after the results have been returned by the TRE, even if your task was submitted another way.
...
- <span style="background-color:blue; border-radius:5px; color:white"> Running </span>: Your task is still in progress - check the task.
- <span style="background-color:green; border-radius:5px; color:white"> Completed </span>: Your task has completed and results are ready to collect.
- <span style="background-color:red; border-radius:5px; color:white"> Failed </span>: Your task has failed.
- <span style="background-color:gold; border-radius:5px; color:white"> Partial result </span>: Your task has failed in at least one of the TREs, and succeeded in at least one.
...
![Screenshot of the event log of a successful task](./event-log.png)
